...
export async function POST(request: Request) {
  try {
    ...
    // 2. Sync to HubSpot (non-blocking — don't fail the request if HubSpot is down)
    ...
    try {
      ...
      const dealName = `${formType === 'general' ? 'Contact' : formType} — ${firstname || ''} ${lastname || ''} (${email})`.trim()
    ...
    // 3. Update Supabase with HubSpot status
    if (hubspotContactId) {
      await supabase
        .from('leads')
