...
export function EditableStat({
  ...
  const currentValue = getContent(contentKey, fallback)
  const [statValue, statLabel] = currentValue.split('|')
  const canEdit = editMode && isOmApexUser
  ...
  const content = (
    <div>
      <div className={valueClassName}>{statValue}</div>
      <div className={labelClassName}>{statLabel}</div>
    </div>
  )
  ...
  return (
    <div className="relative">
      ...
      {isEditing && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30" onClick={() => setIsEditing(false)}>
          <div className="bg-white rounded-lg shadow-xl p-4 w-80 max-w-[90vw]" onClick={(e) => e.stopPropagation()}>
            <div className="mb-2">
              <code className="text-xs bg-gray-100 px-2 py-1 rounded text-brand-primary font-mono">{contentKey}</code>
              <p className="text-xs text-gray-500 mt-1">Format: value|label (e.g., 30+|Years)</p>
            </div>
            <textarea
              value={editValue}
              onChange={(e) => setEditValue(e.target.value)}
